...
class TestStepNames(object):
    def test_format_step_name_none(self):
        sn = StepNames()
        stepname = "A step name without any replacements"
        step = {"name": stepname}
        assert sn.format_step_name(step, [], []) == stepname

    def test_format_step_name_multiple(self):
        sn = StepNames()
    ...
    def test_generator_only_once_when_no_examples(self):
        sn = StepNames()
    ...
    def test_all_steps_all_examples(self):
        sn = StepNames()
